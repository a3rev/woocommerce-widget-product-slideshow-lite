...
class Admin_UI
{
	...
	public function __construct() {
		...
		$this->google_map_api_key_option = WC_PRODUCT_SLIDER_KEY . '_google_map_api_key';
		$this->toggle_box_open_option    = WC_PRODUCT_SLIDER_KEY . '_toggle_box_open';
		$this->version_transient         = WC_PRODUCT_SLIDER_KEY . '_licinfo';

		if ( defined( 'WC_PRODUCT_SLIDER_G_FONTS' ) ) {
			$this->is_load_google_fonts = (boolean) WC_PRODUCT_SLIDER_G_FONTS;
		}
	...
	public function get_premium_video_data() {
		$premium_video_data = array(
				'box_title'    => __( 'Premium Version Enhanced Features', 'woo-widget-product-slideshow' ),
				'image_url'    => WC_PRODUCT_SLIDER_IMAGES_URL. '/video.jpg',
				'video_url'    => 'https://www.youtube.com/embed/9dGw-ORfMIk?version=3&autoplay=1',
				'left_title'   => __( 'Premium Version Enhanced Features', 'woo-widget-product-slideshow' ),
				'left_text'    => __( 'WooCommerce Carousel and Slider', 'woo-widget-product-slideshow' )
	...
	public function get_version_message() {
		...
		if ( false !== $version_transient ) {
			...
			if ( false === $timeout ) {
				$version_message = __( 'You should check now to see if have any new version is available', 'woo-widget-product-slideshow' );
			} elseif ( 'cannot_connect_api' == $version_transient ) {
				$version_message = sprintf( __( 'Connection Failure! Please try again. If this issue persists please create a support request on the plugin <a href="%s" target="_blank">a3rev support forum</a>.', 'woo-widget-product-slideshow' ), $this->support_url );
			} else {
