...
class WC_Product_Slider_Less
{
	...
	public function plugin_build_sass()
    {
        if ( isset( $_POST['form_name_action'] ) ) {
            if ( isset( $_REQUEST['page'] ) && 'wc-product-slider' == $_REQUEST['page'] ) {
                $template_name = '';
                if( (isset($_REQUEST['tab']) && $_REQUEST['tab'] == 'widget-skin') ){
                    $template_name = 'wc_product_slider_widget';
                }
                if(  $template_name != '' ){
                    $sass = $this->sass_content_data( $template_name );
                    $this->plugin_compile_less_mincss( $sass,  $template_name );
                }
            }
        }else{
    ...
    public function sass_content_data( $template_name = '' )
    {
        ...
        ob_start();
        include( $this->plugin_dir. '/includes/'.$template_name.'.php' );
        $sass = ob_get_clean();
